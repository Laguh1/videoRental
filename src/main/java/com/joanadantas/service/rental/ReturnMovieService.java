...
public class ReturnMovieService implements ReturnService{
    ...
    private static final String MOVIE_NOT_RENTED_BYCUSTOMER = "Return operation failed for Movie %s. Movie %s is currently not rented by this customer.";
    ...
    public SuccessfulReturnMessage returnAMovie(String customerId, String movieId) throws CustomException {
        ...
        if(movie.getIsAvailable() || !customer.getMoviesRented().contains(movie)){
            throw new CustomException(String.format(MOVIE_NOT_RENTED_BYCUSTOMER, movie.getTitle(), movie.getTitle()));
        }
